...
│   └── manifest.firefox.json   # MV3 manifest for Firefox (background.scripts + gecko.id)
├── src/
│   ├── background.js      # Toolbar icon click handling (service worker / event page)
...
npm run package:all        # both (after a clean)
```
